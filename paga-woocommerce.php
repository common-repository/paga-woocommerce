...
function tbz_wc_paga_init() {
	...
	class WC_Tbz_Paga_Gateway extends WC_Payment_Gateway {
        ...
        public function admin_options() {
            ...
            $instruction =  '<h3>Paga Payment Gateway</h3>';
            $instruction .= '<p>Paga Payment Gateway allows you to accept payment on your Woocommerce Powered Store Using Paga</p>';
            $instruction .= '<div class="postbox"><div class="inside"><h3>Important Information</h3>';
            $instruction .= '<p>The URL below should be copied and put in the <strong>Payment notification URL</strong> field under the Merchant Information section in the E-Pay Set-up area under your Paga Merchant account.</p>';
            $instruction .= '<code><strong> ' . WC()->api_request_url( 'WC_Tbz_Paga_Gateway' ) . '</strong></code>';
            $instruction .= '<p>This must be done before you start accepting payment using the Paga WooCommerce Payment Gateway plugin.</p></div></div>';
		...
		**/
		public function get_paga_args( $order ) {
	    ...
	    **/
	    public function generate_paga_form( $order_id ) {
		...
		**/
	 	...
	 	**/
		public function get_transaction_message( $transaction_status ){
